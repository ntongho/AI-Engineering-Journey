List Comprehension: A concise, one-line syntax to create a new list from an existing iterable.
Dictionary Comprehension: A concise, one-line syntax to create a new dictionary using key-value expressions.
Set Comprehension: A concise, one-line syntax to create a new set, which automatically excludes duplicate items.
Lambda: A small, anonymous, single-expression function defined without a name using the lambda keyword.
map(): A built-in function that applies a specified function to every item in an iterable.
